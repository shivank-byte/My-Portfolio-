...
height: 66px; overflow: hidden; gap: 10px;
}}
.nav-logo {{
...
.nav-links {{
display: flex; gap: 6px; font-size: 0.83rem; font-weight: 500;
...
/* PROJECT GRID — clean, uniform tiles in a responsive grid (no horizontal
scrolling: everything is visible and reachable by normal vertical scroll,
which reads as more professional than a swipe-strip and works identically
on mobile and desktop). Title is ALWAYS visible in bold crimson serif
below the image (critical: hover doesn't exist on touchscreens, so the
title can never be hidden-until-hover). Hovering on desktop is a pure
bonus: a muted preview video crossfades in over the image and starts
playing, and a secondary panel (category, one-line intro, CTA) expands
below the title. */
.netflix-row {{
display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));
...
display: block; border-radius: 10px; overflow: hidden; background: var(--bg-card);
...
box-shadow: 0 24px 48px -6px rgba(0,0,0,0.5);
z-index: 5;
}}
.netflix-card-img {{ position: relative; width: 100%; aspect-ratio: 4/5; overflow: hidden; background: var(--bg-card2); }}
...
/* Tap-to-preview button — the mobile-usable equivalent of desktop hover,
since touchscreens never fire mouseenter/mouseleave at all. */
.card-play-btn {{
position: absolute; bottom: 8px; right: 8px; z-index: 3;
width: 34px; height: 34px; border-radius: 50%;
background: rgba(15,17,21,0.72); border: 1px solid rgba(255,255,255,0.25);
color: #fff; font-size: 0.85rem; display: flex; align-items: center; justify-content: center;
cursor: pointer; backdrop-filter: blur(2px);
}}
.card-play-btn:active {{ background: var(--red); }}
/* Title: always rendered, never hover-gated. */
.netflix-card-title {{
font-family: var(--serif); font-weight: 700; font-size: 1.08rem; color: var(--red);
line-height: 1.3; padding: 16px 16px 4px;
}}
...
def render_netflix_row(projects):
    """Responsive grid of uniform project tiles. Image + title are ALWAYS
    visible (critical on touchscreens, which have no hover state at all).
    On hover (desktop bonus only): a muted looping preview video crossfades
    in over the image and autoplays, and a secondary info panel (category,
    one-line intro, CTA) expands below the title. Whole tile is still a
    plain <a href='?project=NUM'> link, so tapping on mobile navigates
    ...
    for p in projects:
        ...
        if lead_video:
            ...
            # Tap-to-preview button: hover (onmouseenter/leave below) only ever
            # fires on a real mouse — touchscreens have no hover event at all,
            # so mobile needs its own explicit, tappable trigger instead.
            play_btn_html = """<div class='card-play-btn' onclick="
event.preventDefault(); event.stopPropagation();
var v=this.parentElement.querySelector('.card-video');
if(v.classList.contains('playing')){v.pause(); v.classList.remove('playing'); this.textContent='▶';}
else{v.currentTime=0; v.play(); v.classList.add('playing'); this.textContent='❚❚';}
">▶</div>"""
        hover_js = (
            "onmouseenter=\"var v=this.querySelector('.card-video'); if(v){v.currentTime=0; v.play(); v.classList.add('playing');}\" "
            "onmouseleave=\"var v=this.querySelector('.card-video'); if(v){v.pause(); v.classList.remove('playing'); var b=this.querySelector('.card-play-btn'); if(b){b.textContent='▶';}}\""
            if lead_video else ""
        )
        cards += f"""
<a class='netflix-card' href='?project={p["num"]}#projects' {hover_js}>
<div class='netflix-card-img'>{img_html}{video_html}{play_btn_html}</div>
...
<div class='section-sub'>Applied ML, optimization, and policy work — hover or tap a card for a preview, click through for the full write-up.</div>
</div>
""")
    st.markdown("<div style='padding: 0 clamp(20px, 6vw, 60px);'>", unsafe_allow_html=True)
