...
font-family: var(--mono); font-size: 0.62rem; letter-spacing: 3px;
...
.skill-tile-label {{
font-size: 0.62rem; color: var(--text-mid); text-align: center; line-height: 1.25;
font-weight: 500;
}}
...
/* NETFLIX-STYLE PROJECT ROWS — poster-only by default; hovering both pops
the card AND reveals title/metadata/description that isn't shown otherwise
(the actual Netflix behavior — not just "make an already-labeled card bigger"). */
.netflix-row {{
display: flex; gap: 18px; overflow-x: auto; padding: 24px 4px 140px;
...
.netflix-card:hover {{
/* Real Netflix cards grow in place (no lift), pause briefly before popping
(hover-intent, so a quick mouse pass over doesn't trigger it), and use a
plain dark shadow rather than a brand-colored glow ring. */
...
/* Info panel: zero height / invisible by default (poster-only), expands
open only on hover — this is the actual reveal Netflix does. */
.netflix-card-body {{
max-height: 0; opacity: 0; padding: 0 14px; overflow: hidden;
transition: max-height 0.22s ease 150ms, opacity 0.18s ease 150ms, padding 0.22s ease 150ms;
}}
.netflix-card:hover .netflix-card-body {{ max-height: 190px; opacity: 1; padding: 12px 14px 16px; }}
.netflix-card-eyebrow {{
font-family: var(--mono); font-size: 0.64rem; color: var(--text-dim);
letter-spacing: 1px; text-transform: uppercase; margin-bottom: 6px;
display: flex; align-items: center; gap: 6px; flex-wrap: wrap;
}}
.netflix-card-title {{ font-family: var(--serif); font-weight: 700; font-size: 1.02rem; color: var(--text); line-height: 1.28; margin-bottom: 6px; }}
.netflix-card-intro {{ font-size: 0.8rem; color: var(--text-mid); line-height: 1.5; }}
.netflix-card-cta {{
...
CONSULTING_CASES = [
    {
        ...
        "image": "app/static/edu_case_b64.jpg",
...
<div class='hero-bio'>
With a stats background from BHU and an incoming master's in Operations 
Research at IIT Bombay, I build data-driven tools at the intersection of <strong>policy, 
finance, and applied ML</strong> — from inflation forecasting grounded in RBI 
data to district-level socioeconomic prediction models. A few projects (like 
GeoSphere) reflect broader technical curiosity outside economics.
</div>
<div class='hero-pitch'>
B.Sc. (Hons.) Statistics · Banaras Hindu University<br/>
Incoming M.Sc. Operations Research · IIT Bombay
</div>
<div class='hero-btns'>
...
# ── TICKER ───────────────────────────────────────────────────────────────────
items = ["Python", "Statistics", "Machine Learning", "Economics", "Streamlit", "LangChain", "ARIMA", "Plotly", "Data Science", "IIT Bombay"]
...
""")
    ...
    if videos:
        ...
        cols = st.columns(len(videos)) if len(videos) > 1 else [st]
        for i, vsrc in enumerate(videos):
            label = "▶  Watch demo video" if len(videos) == 1 else f"▶  Watch demo {i+1}"
            container = cols[i] if len(videos) > 1 else st
            if i not in shown:
                if container.button(label, key=f"playvid_{p['num']}_{i}"):
                    shown = set(shown) | {i}
...
<div class='section-sub'>Economics- and finance-relevant work — scroll, hover, click a card for the full write-up.</div>
</div>
""")
    st.markdown("<div style='padding: 0 clamp(20px, 6vw, 60px);'>", unsafe_allow_html=True)
...
<div class='section-sub'>Heavier builds, less economics-adjacent — still worth a close look.</div>
</div>
""")
...
I'm looking for opportunities at the intersection of data science, economics, and real-world impact — consulting, analytics, or research roles where rigorous thinking and engineering skill both count.<br/>
