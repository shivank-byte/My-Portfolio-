...
/* BLOB PHOTO + DATA ART */
.photo-zone {{ position: relative; width: 100%; max-width: 340px; margin: 0 auto; }}
...
.data-panel {{
margin-top: 20px; padding: 18px 18px 14px;
...
.data-panel svg {{ width: 100%; height: 110px; display: block; }}
...
.dp-bar:nth-child(2) {{ animation-delay: 0.15s; }}
.dp-bar:nth-child(3) {{ animation-delay: 0.3s; }}
.dp-bar:nth-child(4) {{ animation-delay: 0.45s; }}
.dp-bar:nth-child(5) {{ animation-delay: 0.6s; }}
@keyframes dp-grow {{ 0% {{ transform: scaleY(0.55); }} 100% {{ transform: scaleY(1); }} }}
.dp-line {{
...
.dp-dot {{ animation: dp-pulse 1.8s ease-in-out infinite; }}
@keyframes dp-pulse {{ 0%,100% {{ opacity: 0.4; r: 3; }} 50% {{ opacity: 1; r: 5; }} }}
...
.proj-editorial:hover .proj-num {{ color: rgba(224,52,42,0.25); transform: scale(1.05); }}
...
.proj-mini-stat-label {{
    font-size: 0.68rem; color: var(--text-dim); margin-top: 2px; line-height: 1.3;
}}
...
.status-live    {{ background: rgba(34,197,94,0.12); color: #4ade80; border: 1px solid rgba(74,222,128,0.3); animation: live-pulse 2.5s ease-in-out infinite; }}
...
/* Animated gradient border on project hover */
.proj-editorial {{
position: relative;
}}
.proj-editorial::after {{
content: '';
position: absolute; bottom: 0; left: 0; right: 0; height: 1px;
background: linear-gradient(90deg, transparent, var(--red), transparent);
opacity: 0; transition: opacity 0.4s;
}}
.proj-editorial:hover::after {{ opacity: 1; }}
...
.minor-card[open] {{ flex-basis: 300px; border-style: solid; background: var(--bg-card); }}
.minor-card:hover {{ border-color: rgba(224,52,42,0.3); }}
...
.minor-hint {{ font-family: var(--mono); font-size: 0.66rem; color: var(--text-dim); letter-spacing: 1px; text-align: center; margin: -8px 0 14px; }}
...
.skill-block:hover {{ border-color: rgba(224,52,42,0.3); transform: translateY(-2px); box-shadow: 0 8px 28px rgba(0,0,0,0.25); }}
...
.section-label {{ animation: fade-up 0.5s ease both; }}
...
color: #f5f4f2; margin-bottom: 14px; letter-spacing: -1px;
}}
.contact-heading em {{ color: var(--red); font-style: italic; font-weight: 400; }}
.contact-sub {{ font-size: 1rem; color: rgba(245,244,242,0.45); margin-bottom: 8px; font-style: italic; font-family: var(--serif); }}
.contact-cta {{
font-size: 0.84rem; color: rgba(245,244,242,0.3); margin-bottom: 36px;
...
.contact-link:hover {{ background: var(--red); color: white; transform: translateY(-2px); }}
...
.footer-left {{ font-family: var(--mono); font-size: 0.72rem; color: rgba(255,255,255,0.22); letter-spacing: 1px; }}
.footer-center {{
...
.footer-icon:hover {{ border-color: var(--red); color: var(--red); background: rgba(224,52,42,0.08); }}
.footer-right {{ font-family: var(--serif); font-size: 0.8rem; color: rgba(255,255,255,0.18); font-style: italic; }}
...
MAJOR_PROJECTS = [
    ...
    },
    {
        ...
        "status": "live",
        "status_label": "Live",
        "desc": "A 21-module interactive Earth Science intelligence platform built on the BHU NEP 4-year syllabus (~3,800 lines, single file). Covers the complete undergraduate curriculum: mineralogy, petrology, structural geology, stratigraphy, geomorphology, Indian rivers, oceanography, earthquake monitoring, economic geology, and more. Live USGS earthquake data, Wikipedia photo API for real rock/mineral/volcano photographs, Gemini AI integration with 60+ topic fallback, and a field diary capturing the lived experience of geology at BHU.",
        "features": "Interactive 3D Tectonic Globe · Multi-Module Earth Science Intelligence Engine · Flawless Mobile-Optimized Dashboard Design · Live USGS M4.5+ earthquake feed · Wikipedia REST API photos · Gemini AI geology Q&A · BHU NEP syllabus alignment across all 8 semesters · 45+ quiz questions · 24 flashcards with mastery tracking",
        "pitch": "Built by one student, using only free tools, in parallel with a full academic semester. No team. No budget. No dedicated server. I handled the system architecture, data modeling, UX design, and module integration across 21 independent sections — leveraging AI to accelerate generation and debugging of modular components while owning the full technical direction.",
    ...
    },
    {
        ...
        "status": "live",
        "status_label": "Live",
        "desc": "A single structured prompt acting simultaneously as ATS scorer, senior FAANG recruiter critic, and resume rewriter — powered by Gemini 2.5 Flash. Scored 92/100 ATS compatibility and 85/100 job match on a real resume test. The anti-fabrication guardrail forces the model to write [X%] instead of inventing metrics, mirroring the same epistemic discipline required in statistical research.",
        "features": "Single-call structured JSON output · Anti-hallucination bracket guardrail · PDF/DOCX/TXT parsing · ATS score + job-match score + bullet rewrites + full tailored resume rewrite in one pass",
        "pitch": None,
...
MINOR_PROJECTS = [
    ...
    },
    {
        ...
        "category": "Business Analytics · Data Engineering",
        "status": "wip", "status_label": "Working On",
        "desc": "A complete, reproducible Indian banking analytics system covering 7 pillars across 18 RBI datasets (FY1996–2026): sectoral credit deployment, priority sector lending, CASA ratios, NPA trends, digital payments (UPI/IMPS/NEFT), financial inclusion (SHG linkage), and state-wise credit-deposit ratios — overlaid with macro context (GDP, CPI, repo rate). Key finding: NPA peaked at 11.2% in FY2017-18 and recovered to 2.75% by FY2023-24 — the most significant balance sheet healing in Indian banking in two decades. UPI grew ~10,000x in transaction volume since launch. Six states have CD ratios above 90%, signaling geographic credit concentration risk.",
        "stack": ["Python", "Pandas", "openpyxl", "Tableau", "Plotly", "RBI DBIE API"],
    },
    {
...
""")
    ...
    # Small, always-playing animated data graphic — CSS/SVG only, no tap-to-play needed.
    svg_color = "#e0342a"
    axis_color = "rgba(150,150,150,0.35)"
    html(f"""
<div class='photo-zone data-panel'>
<svg viewBox='0 0 300 110' xmlns='http://www.w3.org/2000/svg'>
<line x1='10' y1='95' x2='290' y2='95' stroke='{axis_color}' stroke-width='1'/>
<rect class='dp-bar' x='30'  y='40' width='16' height='55' rx='2' fill='{svg_color}' opacity='0.85'/>
<rect class='dp-bar' x='55'  y='25' width='16' height='70' rx='2' fill='{svg_color}' opacity='0.7'/>
<rect class='dp-bar' x='80'  y='50' width='16' height='45' rx='2' fill='{svg_color}' opacity='0.85'/>
<rect class='dp-bar' x='105' y='15' width='16' height='80' rx='2' fill='{svg_color}' opacity='0.7'/>
<rect class='dp-bar' x='130' y='35' width='16' height='60' rx='2' fill='{svg_color}' opacity='0.85'/>
<polyline class='dp-line' points='165,70 185,50 205,60 225,30 245,42 265,20'
fill='none' stroke='{svg_color}' stroke-width='2.5'/>
<circle class='dp-dot' cx='265' cy='20' r='4' fill='{svg_color}'/>
<text x='168' y='104' font-size='8' fill='{axis_color}' font-family='monospace'>CPI forecast</text>
<text x='30' y='104' font-size='8' fill='{axis_color}' font-family='monospace'>82.4% acc.</text>
</svg>
</div>
<div class='data-panel-label'>◈ DATA · STATISTICS · ML</div>
""")
html("""
<div class='stat-row'>
...
""")
        with col_globe:
            ...
            fig_globe.add_trace(go.Scattergeo(
                lon=[68,72,77,80,88,97,92,80,77,72,68],
                lat=[23,20, 8, 8, 8,28, 8, 8, 8,20,23],
                mode="lines", line=dict(width=1.5, color="rgba(224,52,42,0.4)"),
                name="India", hoverinfo="skip",
            ))
            # Himalayan arc
            fig_globe.add_trace(go.Scattergeo(
                lon=[73,77,80,84,88,92,96],
                lat=[34,33,30,28,27,27,28],
                mode="lines", line=dict(width=2, color="rgba(224,52,42,0.7)", dash="dot"),
                name="Himalayan Belt", hoverinfo="skip",
            ))
            # Seismic zones (BIS 1893)
            zone_colors = {"Zone V (Very High)":"#e0342a","Zone IV (High)":"#f97316",
                           "Zone III (Moderate)":"#facc15","Zone II (Low)":"#4ade80"}
            zone_lons = {"Zone V (Very High)":[93.9,91.7,88.4,77.2,74.8,76.9],
                         "Zone IV (High)":[88.3,85.3,80.9,77.0,72.8,76.0],
                         "Zone III (Moderate)":[78.5,72.5,68.0,80.0,85.0,76.0],
                         "Zone II (Low)":[77.6,74.0,79.0,83.0]}
            zone_lats = {"Zone V (Very High)":[24.8,26.2,27.4,33.7,34.1,34.5],
                         "Zone IV (High)":[22.6,23.5,26.2,28.6,19.0,30.5],
                         "Zone III (Moderate)":[17.4,23.0,22.5,21.0,20.0,15.0],
                         "Zone II (Low)":[13.0,15.0,12.0,14.0]}
            zone_sizes = {"Zone V (Very High)":10,"Zone IV (High)":8,
                          "Zone III (Moderate)":6,"Zone II (Low)":5}
            for z in zone_colors:
                fig_globe.add_trace(go.Scattergeo(
                    lon=zone_lons[z], lat=zone_lats[z], mode="markers",
                    marker=dict(size=zone_sizes[z], color=zone_colors[z], opacity=0.88,
            ...
            fig_globe.update_layout(
                geo=dict(
                    ...
                    projection_rotation=dict(lon=80, lat=22, roll=0),
                    showland=True, landcolor="#1a2d1a" if DM else "#c8e6c9",
                    showocean=True, oceancolor="#0d1b2a" if DM else "#cce4f5",
                    showlakes=False,
                ...
                legend=dict(
                    font=dict(color="#f5f4f2",size=9,family="JetBrains Mono"),
        ...
        html("<div style='border-bottom:1px solid var(--rule);margin:0;'></div>")
    elif p.get("inline_chart"):
        col_meta, col_chart = st.columns([1, 1])
        with col_meta:
            html(f"""
<div style='padding: 52px 0 52px;'>
<div class='proj-eyebrow'>{p["num"]} &nbsp;·&nbsp; {p["category"]} &nbsp;·&nbsp; <span class='status-badge {status_class[p["status"]]}'>{p["status_label"]}</span></div>
...
""")
            if p.get("pitch"):
                with st.expander("◈ The engineering insight"):
                    st.markdown(p["pitch"])
        with col_chart:
            bg_color = "#0a0a0c" if DM else "#f8f7f4"
            months = ["Jan'22","Mar","May","Jul","Sep","Nov",
                      "Jan'23","Mar","May","Jul","Sep","Nov",
                      "Jan'24","Mar","May","Jul","Sep","Nov",
                      "Jan'25","Mar"]
            values = [0.52,0.68,0.71,0.46,0.81,0.49,
                      0.54,0.31,0.62,0.55,0.44,0.38,
                      0.44,0.50,0.43,0.33,0.51,0.45,
                      0.28,0.36]
            fig_chart = go.Figure()
            fig_chart.add_trace(go.Scatter(
                x=months, y=values, mode="lines+markers",
                line=dict(color="#e0342a", width=2),
                marker=dict(size=5, color="#e0342a"),
                fill="tozeroy", fillcolor="rgba(224,52,42,0.08)",
                name="CPI MoM %",
            ))
            fig_chart.add_hline(
                y=sum(values)/len(values), line_dash="dot",
                line_color="rgba(245,244,242,0.2)",
                annotation_text="Mean",
                annotation_font_color="rgba(245,244,242,0.3)",
            ...
            fig_chart.update_layout(
                ...
                showlegend=False,
                title=dict(text="India CPI MoM % Change — forecast training series",
                           font=dict(color="rgba(245,244,242,0.4)",size=10,family="JetBrains Mono")),
                xaxis=dict(showgrid=False,tickfont=dict(color="rgba(245,244,242,0.3)",size=9),tickangle=-45),
                yaxis=dict(showgrid=True,gridcolor="rgba(255,255,255,0.05)",
                           tickfont=dict(color="rgba(245,244,242,0.3)",size=9),
                           title=dict(text="%",font=dict(color="rgba(245,244,242,0.3)",size=9))),
            )
            st.plotly_chart(fig_chart, use_container_width=True, config={"displayModeBar": False})
            html("""
<div style='background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:14px 16px;margin-top:12px;'>
<div style='font-family:monospace;font-size:0.68rem;color:rgba(245,244,242,0.35);letter-spacing:2px;margin-bottom:10px;'>MODEL ACCURACY — 12-MONTH HELD-OUT TEST SET</div>
<table style='width:100%;border-collapse:collapse;font-family:monospace;font-size:0.8rem;'>
<tr style='color:rgba(245,244,242,0.3);border-bottom:1px solid rgba(255,255,255,0.06);'><td style='padding:5px 0;'>Model</td><td>MAE</td><td>RMSE</td><td>Note</td></tr>
<tr style='color:#4ade80;'><td style='padding:5px 0;'>Prophet</td><td>0.4761</td><td>0.6518</td><td>✓ Best</td></tr>
<tr style='color:rgba(245,244,242,0.55);'><td style='padding:5px 0;'>XGBoost</td><td>0.5962</td><td>0.7697</td><td></td></tr>
<tr style='color:rgba(245,244,242,0.55);'><td style='padding:5px 0;'>Linear AR</td><td>0.6261</td><td>0.7867</td><td>Baseline</td></tr>
<tr style='color:rgba(245,244,242,0.55);'><td style='padding:5px 0;'>ARIMA</td><td>0.7077</td><td>0.8868</td><td></td></tr>
</table>
</div>
""")
        html("<div style='border-bottom:1px solid var(--rule);margin:0;'></div>")
...
"""
html(f"<div class='minor-scroll'>{minor_cards_html}</div>")
st.markdown("</div>", unsafe_allow_html=True)
...
<div class='contact-heading'>Let's build something<br/><em>that matters.</em></div>
<div class='contact-sub'>Incoming M.Sc. Economics · IIT Kanpur · 2025</div>
<div class='contact-cta'>
I'm looking for opportunities at the intersection of data science, economics, and real-world impact —<br/>
consulting, analytics, or research roles where rigorous thinking and engineering skill both count.<br/>
Open to coffee chats, project collaboration, or campus recruiting at IIT Kanpur.
</div>
...
<a class='footer-icon' href='mailto:thakuryshivank143@gmail.com' title='Email'>✉</a>
<a class='footer-icon' href='https://github.com/shivank-byte/' target='_blank' title='GitHub'>⌥</a>
<a class='footer-icon' href='https://www.linkedin.com/in/shivank-byte/' target='_blank' title='LinkedIn'>in</a>
</div>
<div class='footer-right'>Shivank Thakur · IIT Kanpur</div>
</div>
""")
