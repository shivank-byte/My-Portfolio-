...
<div class='nav-bar'>
    ...
    <div class='nav-tag'>◈ {CURRENT_YEAR}</div>
</div>
""", unsafe_allow_html=True)

# ── HERO ─────────────────────────────────────────────────────────────────────
col_text, col_photo = st.columns([2.4, 1])

with col_text:
    st.markdown("""
    <div class='hero'>
        <div class='hero-eyebrow'>Portfolio · Data &amp; Statistics</div>
        <div class='hero-name'>Shivank <span>Thakur</span></div>
        <div class='hero-title'>Statistics, data, and decisions</div>
        <div class='hero-bio'>
            ...
            Indian datasets. Background in B.Sc. (Hons.) Statistics from Banaras
            Hindu University.
        </div>
        <div class='hero-rule'></div>
        <div class='hero-meta'>
            <div class='hero-meta-item'><b>Focus</b> · Applied Statistics &amp; ML</div>
            <div class='hero-meta-item'><b>Based in</b> · India</div>
            <div class='hero-meta-item'><b>Status</b> · Open to opportunities</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
with col_photo:
...
<div class='section' id='projects'>
    ...
    <div class='section-sub'>Live work first, work-in-progress after — numbered in the order you'll most likely want to look at them.</div>
</div>
""", unsafe_allow_html=True)
...
for i, p in enumerate(PROJECTS):
    with proj_cols[i % 2]:
        stack_html = "".join(f"<span class='stack-tag'>{s}</span>" for s in p["stack"])
        link_html = f"<a class='proj-link' href='{p['link']}' target='_blank'>Visit project ↗</a>" if p.get("link") else ""
        st.markdown(f"""
        <div class='proj-card'>
            <div class='proj-number'>{p['id']} · {p['highlight']}</div>
...
st.markdown("""
<div class='section section-alt' id='flagship'>
    <div class='section-label'>◈ Flagship</div>
    <div class='section-heading'>GeoSphere India</div>
    <div class='section-sub'>The most complete project here — try the interactive earthquake-risk lookup below.</div>
</div>
""", unsafe_allow_html=True)
...
with fcol2:
    ...
    # Genuine interactive element: a simple region risk lookup
    region = st.selectbox(
    ...
    <div class='demo-shell'>
        <div class='flagship-info-label' style='color:var(--text-dim);'>SEISMIC CONTEXT</div>
...
<div class='section' id='skills'>
    ...
    <div class='section-sub'>Grouped by what they're for, not where they sit in a resume.</div>
</div>
""", unsafe_allow_html=True)
...
st.markdown(f"<div class='skills-panel'>{skill_rows_html}</div>", unsafe_allow_html=True)

st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True)

# ── CONSULTING CASE STUDIES ──────────────────────────────────────────────────
st.markdown("""
<div class='section section-alt' id='consulting'>
    <div class='section-label'>◈ Applied Thinking</div>
    <div class='section-heading'>Case Studies</div>
    <div class='section-sub'>Real consulting-style engagements — problem framing through to a roadmap. Case 1 is in progress; cases 2 and 3 are reserved for upcoming work.</div>
</div>
""", unsafe_allow_html=True)
...
for c in CONSULTING_CASES:
    if c["placeholder"]:
        ...
        <div class='consult-card consult-placeholder'>
            <div class='consult-image' style='background:var(--paper);display:flex;align-items:center;justify-content:center;'>
                <span style='font-size:1.8rem;color:var(--rule);'>◈</span>
            </div>
...
EMAIL_FULL = f"{EMAIL_USER}@{EMAIL_DOMAIN}"
